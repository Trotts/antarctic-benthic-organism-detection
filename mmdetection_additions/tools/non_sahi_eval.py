...
from sahi_eval import (
    load_results,
    build_whole_image_dataset,
    conf_mat,
...
def parse_args():
    ...
    parser.add_argument(
        "output_folder_name",
        help="Name of the output folder. Stored in the model directory.",
    )
    parser.add_argument(
        "--bbox-conf-threshold",
        type=float,
    ...
    parser.add_argument(
        "--abundance-ordering", type=str, help="Path to the abundance ordering pkl"
    )
...
def get_whole_image_filepaths(whole_images_src, dataset_json):
    ...
    dataset_image_filepaths = [
        whole_images_src + "/" + image["file_name"] for image in dataset_json["images"]
    ]
        ...
        for patch_results in results_per_image_by_id[per_img_results]:
            if len(patch_results["bboxes"]) > 0:
                for i, bbox in enumerate(patch_results["bboxes"]):
                    ...
                    per_img_results_mmdet["pred_instances"]["labels"].append(
                        int(patch_results["labels"][i])
                    )
                    per_img_results_mmdet["pred_instances"]["scores"].append(
                        float(patch_results["scores"][i])
                    )

        # Make bboxes, labels, scores a torch tensor
        per_img_results_mmdet["pred_instances"]["bboxes"] = torch.tensor(
            per_img_results_mmdet["pred_instances"]["bboxes"]
        )
        per_img_results_mmdet["pred_instances"]["labels"] = torch.tensor(
            per_img_results_mmdet["pred_instances"]["labels"]
        )
        per_img_results_mmdet["pred_instances"]["scores"] = torch.tensor(
            per_img_results_mmdet["pred_instances"]["scores"]
        )
                ...
                results_out.append(
                    {
                        ...
                        "category_name": dataset.metainfo["classes"][
                            result["labels"][i]
                        ],
                    }
                )
...
def writePNGwithdpi(im, filename, dpi=(300, 300)):
    ...
    pHYs = b"pHYs" + struct.pack(
        "!IIc", int(dpi[0] / 0.0254), int(dpi[1] / 0.0254), b"\x01"
    )
...
def visualize_object_predictions(
    ...
    if output_dir is not None:
        ...
        writePNGwithdpi(
            cv2.cvtColor(image, cv2.COLOR_RGB2BGR), save_path, dpi=(300, 300)
        )
...
def visualise_results(
    ...
    colours = [
        (int(255 * colour[0]), int(255 * colour[1]), int(255 * colour[2]))
...
def main():
    ...
    inference_model = DetInferencer(
        args.config_src, args.checkpoint_src, device="cuda", show_progress=False
    )
    ...
    for image_src in whole_image_filepaths:
        ...
        results_per_patch = get_per_patch_results(
            patch_bboxes, img, inference_model, args.bbox_conf_threshold
        )
    ...
    confusion_matrix = conf_mat(
        dataset, results_mmdet, score_thr=args.bbox_conf_threshold, tp_iou_thr=0.5
    )

    confusion_matrix, class_ordering = reorder_confusion_matrix(
        confusion_matrix, dataset, args.abundance_ordering
    )
    ...
    visualise_results(
        whole_image_filepaths, results_per_image_by_id, out_full, image_ids, dataset
    )
