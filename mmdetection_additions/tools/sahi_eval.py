...
from confusion_matrix_abundance_ordered import (
...
def parse_args():
    ...
    parser.add_argument(
        "--postprocess-type", type=str, default="NMM", help="Postprocessing type"
    )
    parser.add_argument(
        "--match-threshold", type=float, default=0.5, help="Match threshold for SAHI"
    )
...
def build_whole_image_dataset(cfg_src, whole_image_test_dataset_json_path):
    ...
    cfg.test_dataloader.dataset.data_root = whole_image_test_dataset_json_path.split(
        "annotations"
    )[0]
...
def prepare_results_for_mmdet(results_grouped):
    ...
    for per_img_res in results_grouped:
        ...
        per_img_res_mmdet["pred_instances"]["scores"] = torch.tensor(
            [item["score"] for item in results_grouped[per_img_res]]
        )
        per_img_res_mmdet["pred_instances"]["labels"] = torch.tensor(
            [item["category_id"] for item in results_grouped[per_img_res]]
        )
...
def reorder_confusion_matrix(conf, dataset, abundance_ordering):
    ...
    for key1, i in class_ordering.items():
        for key2, j in class_ordering.items():
            confusion_matrix_reordered[i, j] = conf[
...
def plot_conf_matrix(
    ...
    """
    plot_confusion_matrix(
        confusion_matrix, labels, show=True, title=title, save_dir=out_full
    )
...
def main():
    ...
    conf = conf_mat(
        dataset,
        results_grouped_mmdet,
        score_thr=args.bbox_conf_threshold,
        tp_iou_thr=args.iou_threshold,
    )

    if args.abundance_ordering is not None:
        confusion_matrix_reordered, class_ordering = reorder_confusion_matrix(
            conf, dataset, args.abundance_ordering
        )
        plot_conf_matrix(
            confusion_matrix_reordered,
            class_ordering.keys(),
            f"Normalised Confusion Matrix\nSAHI processed ({args.postprocess_type}), ordered by abundance",
            out_full,
        )
    else:
        ...
        plot_conf_matrix(
            conf,
            labels,
            f"Normalised Confusion Matrix\nSAHI processed ({args.postprocess_type})",
            out_full,
        )
