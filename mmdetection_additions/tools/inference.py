...
def parse_args():
    ...
    parser.add_argument('output', type=str, default='output', help='Directory to save results and optional visualisations')
    parser.add_argument('patch_size', help='Patch size used for training')
    parser.add_argument('--bbox-conf-threshold', type=float, default=0.6, help='Bounding box confidence threshold for filtering predictions')
    parser.add_argument('--overlap', type=float, default=0.5, help='Overlap ratio for patching images')
    parser.add_argument('--post-process-match-threshold', type=float, default=0.2, help='Threshold for post-processing matching')
    parser.add_argument('--post-process-algo', type=str, default='NMM', help='Post-processing algorithm to use')
    parser.add_argument('--visualise', action='store_true', help='Whether to visualise the results')
    
    return parser.parse_args()

def run_inference(model, input_path, patch_height,
                  patch_width, overlap, post_process_type,
...
def export_coco_visualisations(results_sahi, file_path, ids_to_class,
                               palette, output_dir, figsize=(30, 30), 
                               fontsize=12):
    """
